...
In the GAMs created in this vignette and this package, the Intercept is treated as an explicitly addressable term. By way of example, consider the model `m` created below:
```{r eval = F}
...
```{r m1, eval = T, echo = T, cache = cache.val}
# initial model
m1 <- gam(
  ndvi ~ 0 +
...
```{r appm1, eval = T, echo = T, cache = cache.val, fig.cap = "Diagnostic plots of `m1`."}
...
```{r m2, eval = T, echo = T, cache = cache.val}
...
```{r m3, eval = T, echo = T, cache = cache.val}
...
```{r m4, eval = T, echo = T, cache = cache.val}
...
```{r m5, eval = T, echo = T, cache = cache.val}
...
```{r m6, eval = T, echo = T, cache = cache.val}
...
```{r vcs, eval = T, echo = T, cache = cache.val}
...
```{r svccoefs2, eval = T, echo = T, cache = cache.val}
# create time slices
